            ...
            public boolean mousePressed(MouseButton button) {
                if(button.isRight() && search.isMouseOver() && !listOfSearch.isEmpty()){
                    ...
                    listOfSearch.forEach(s -> {
                        contextMenuItems.add(new ContextMenuItem(Component.literal(s), ItemIcon.getItemIcon(ItemStack.EMPTY), () -> {
                            search.setText(s);
                            infoTabPanel.refreshWidgets();
                        }));
                    });
...
//            if (BestiaryScreen.this.search.getText().isEmpty()){
//                if(widget instanceof ListTabButton button)
//                    if(DataBase.GROUPCONTENT.containsKey(button.content.getGroup().id))
//                        for (Map.Entry<String, Group> group:DataBase.GROUPCONTENT.entrySet()) {
//                            if(!group.getValue().collapse){
//                                super.add(widget);
//                            } else super.add(widget);
//                        }
//                    else super.add(widget);
//                else super.add(widget);
//            } else if(widget.getTitle().getString().toLowerCase().contains(BestiaryScreen.this.search.getText().toLowerCase())){
//                super.add(widget);
//            }
...
//            }
            ...
            if(content instanceof TableInfo info){
                ...
                panel.setSize(info.sizeTittle + info.sizeContent, panel.getHeight() + info.h);
            ...
            widgets.forEach(s -> {
                if(s instanceof TextPanel panel){
                    if(panel.infoContent instanceof HeaderInfo info){
                        if (info.isCenter) {
                            panel.setPosAndSize((this.getWidth() / 2) + info.x, posY + info.y, info.w, info.h);
